...
, Final AS (
   ...
   , CASE WHEN (year(from_unixtime(coalesce(rd.pickup_time,t1.scheduled_pickup_timestamp))) = 1970) THEN from_unixtime(t3.order_arrange_shipping_timestamp)
    ...
    , case
    when t1.log_status = 1000 then 'NEW'
    when t1.log_status in (1001,1040) then 'INFO_RECEIVED'
    when t1.log_status in (1061,1051,2001,2008,2009,2015,2030,2040,3001,3060,3080,3090) then 'IN_TRANSIT'
    when t1.log_status in (9997,3099,3070) then 'EXCEPTION'
    when t1.log_status = 9999 then 'CANCELLED'
    when t1.log_status = 1050 then 'DOMESTIC_PICKUP'
    when t1.log_status in  (4010,4020,4100) then 'DOMESTIC_SORTING'
    when t1.log_status = 4060 then 'DOMESTIC_DELIVERING'
    when t1.log_status = 4097 then 'DOMESTIC_DELIVERED'
    when t1.log_status = 4070 then 'DELIVERY_PENDING'
    when t1.log_status = 9998 then 'PARCEL_LOST'
    when t1.log_status =  8020 then 'PARCEL_RETURN'
    when t1.log_status = 8050 then 'RETURN_COMPLETED'
    when t1.log_status = 8099 then 'RETURN_FAILED'
    when t1.log_status = 8045 then 'PENDING_FOR_RETURN'
    when t1.log_status in (4098,8001,4102) then 'REQUEST_RETURN'
    when t1.log_status in (996,1041,8010) then 'PICKUP_RETRY'
    when t1.log_status in (1099,2099,9995) then 'DOMESTIC_PICKUP_FAILED'
    when t1.log_status = 1060 then 'DOMESTIC_DROPOFF'
    when t1.log_status = 1042 then 'DOMESTIC_PICKUP_PENDING'
    when t1.log_status in (8000,8040) then 'RETURN_STARTED'
    when t1.log_status = 4101 then 'UPDATE_STORE_SUBMITTED'
    when t1.log_status = 4103 then 'UPDATE_STORE_CANCEL'
    when t1.log_status = 9994 then 'IMLM_DISPOSED'
    when t1.log_status = 8060 then 'PARCEL_CONFISCATED'
    when t1.log_status = 9993 then 'TERMINATE_AS_DAMAGE'
    else 'no'
  end as sls_status
...
, round(approx_percentile(((to_unixtime(pku_1st_time) - f.Logistics_request_created)/86400),0.95),3) gap_time
...
--and
 f.shipment_type1 = 'pickup'
and date(scheduled_date1) between date '2022-10-01' and date '2022-10-31'
and o.seller_shipping_address_state in ('Hà Nội','TP.Hồ Chí Minh')
GROUP BY 1,2,3
