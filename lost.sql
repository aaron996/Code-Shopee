...
,min(case when resource_status = '21' then from_unixtime((CASE WHEN (ctime > actual_time) THEN (actual_time - 3600) ELSE (ctime - 3600) END)) end ) pickup_failed_time2
,min(case when resource_status in ('19','20') then from_unixtime((CASE WHEN (ctime > actual_time) THEN (actual_time - 3600) ELSE (ctime - 3600) END)) end ) pickup_done_time2
FROM
...
--,t1.charged_weight parcel_chargeable_weight
...
--,sls.deli_1st_time deli_1st_time
, l.lost_update_time
...
where 
fulfilment_channel_id in (50015,50016)
and to_unixtime(sls.pickup_failed_time2) > to_unixtime(sls.pickup_done_time2)
and o.logistics_status_id = 2
limit 100
