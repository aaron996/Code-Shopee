...
( select distinct lm_tracking_no
        ...
        --, cast((date_diff('hour',pickup_done_time, first_attempt_delivery_time)) as double)/24.00 as delivery_leadtime
        -- , cast((case when total_sla is null then (case when buyer_zone = 'Urban' then (3.2 + d_sundays + d_holidays)
        ...
        --       end) as double) final_sla
        --, cast((total_sla + d_sundays + d_holidays) as double) final_sla
        , route
        , fulfilment_channel_id
        , total_sla
...
select distinct lm_tracking_no
        ...
        , "tpl_call"
        , first_attempt_delivery_time
        ,case --when fulfilment_channel_id = 50023 and ((lower(Seller_State) = lower(Buyer_State) and buyer_zone = 'Rural') 
        --                                                 or (lower(Seller_State) <> lower(Buyer_State) and Buyer_State in ('Hà Nội','TP. Hồ Chí Minh') and buyer_zone = 'Urban')
        --                                                 or (route = 'Inter Region' and buyer_zone = 'Urban')) 
        --             and hour(pickup_done_time) <= 11 and date(first_attempt_delivery_time) <= date(expected_delivery) then 'Ontime'
        --     when fulfilment_channel_id = 50023 and ((lower(Seller_State) = lower(Buyer_State) and buyer_zone = 'Rural') 
        --                                                 or (lower(Seller_State) <> lower(Buyer_State) and Buyer_State in ('Hà Nội','TP. Hồ Chí Minh') and buyer_zone = 'Urban')
        --                                                 or (route = 'Inter Region' and buyer_zone = 'Urban')) 
        --             and hour(pickup_done_time)  >= 12 and date(first_attempt_delivery_time) < date(expected_delivery) then 'Ontime'
        --     when fulfilment_channel_id = 50023 and ((lower(Seller_State) = lower(Buyer_State) and buyer_zone = 'Rural') 
        --                                                 or (lower(Seller_State) <> lower(Buyer_State) and Buyer_State in ('Hà Nội','TP. Hồ Chí Minh') and buyer_zone = 'Urban')
        --                                                 or (route = 'Inter Region' and buyer_zone = 'Urban')) 
        --             and hour(pickup_done_time)  >= 12 and date(first_attempt_delivery_time) = date(expected_delivery) and hour(first_attempt_delivery_time) <= 13 then 'Ontime'
        --     when fulfilment_channel_id = 50023 and hour(pickup_done_time) <= 11 and date(first_attempt_delivery_time) < date(expected_delivery) then 'Ontime'
        --     when fulfilment_channel_id = 50023 and hour(pickup_done_time) <= 11 and date(first_attempt_delivery_time) = date(expected_delivery) and hour(first_attempt_delivery_time) <= 13 then 'Ontime'
        --     when fulfilment_channel_id = 50023 and hour(pickup_done_time) >= 12 and date(first_attempt_delivery_time) <= date(expected_delivery) then 'Ontime'
        ...
        ,expected_delivery
        from base2 as b 
        where date(expected_delivery) between date '2023-02-13' and date '2023-02-19'
        and fulfilment_channel_id = 50015
        and Buyer_State = 'Hà Nội'
)

    Select week(expected_delivery) week_
    ,lm_tracking_no
        , Buyer_State
        , Buyer_City
        , "tpl_call"
        , route
,Check_delivery_ontime        
        
        
From base3
--Group by 1,2,3,4,10--,5--,6,7--,8--,9
