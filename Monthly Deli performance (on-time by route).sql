...
select
concat(cast(month(expected_delivery)as varchar ),'/',cast(year(expected_delivery)as varchar )) month_
    ,route
    ,count(lm_tracking_no) as vol
    --,round(approx_percentile(cast((to_unixtime (deli_1st_time) - to_unixtime(pku_done_time)) as double)/24/3600,0.95),1) as deli_aging
...
from data1 
where date(expected_delivery) between date '2022-06-01' and date '2023-01-31'
