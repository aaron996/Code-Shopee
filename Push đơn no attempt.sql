...
select distinct
      ...
      ,st.pku_done_time pku_done_time
...
left join holiday d on lower(o.fulfilment_shipping_carrier) = lower(d.shipping_carrier) and (cast(d.holiday_start as date) between date(st.pku_done_time) and date(st.deli_1st_time))
...
shipping_carrier "Đơn vị vận chuyển"
    ,lm_tracking_no "Mã vận đơn"
    ,seller_shipping_address_state "Tỉnh thành người gửi"
    ,seller_shipping_address_city "Quận huyện người gửi"
    ,buyer_shipping_address_state "Tỉnh thành người mua"
    ,buyer_shipping_address_city "Quận huyện người mua"
    ,route "Tuyến"
    --,case when date(deli_1st_time) <= date(expected_delivery) then 'Giao đúng hẹn' else 'Trễ giao' end "Check trễ giao"
    ,case 
     when date(deli_failed_time) is not null then 'Phát hoàn thành công' 
     when date(deli_done_time) is not null then 'Đơn giao thành công' 
     else 'Đơn tồn giao' end "Check trễ giao"
    ,pku_done_time "Thu gom thành công"
    ,date(deli_1st_time) "Giao hàng lần đầu"
    ,date(expected_delivery) "Ngày đến hạn giao hàng"
    ,sla "SLA"
    ,d_sundays "Ngày cuối tuần"
    ,d_holidays "Ngày nghỉ lễ"
...
from data1 
where date(expected_delivery) between (current_date - interval '10' day) and (current_date + interval '1' day)
and shipping_carrier in ('VNPost Nhanh','VNPost Tiết Kiệm')
and deli_1st_time is null
and deli_failed_time is null 
and deli_done_time is null 
and pku_done_time is not null 
and logistics_status_id = 2
--group by 1,2,3,4,5
