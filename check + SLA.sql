...
select distinct
      ...
      case 
      when 
      t1.fulfillment_channel_id = 50015 
      and sl.route = 'Intra City' 
      --and sl.delivery_area = 'Urban' 
      then cast((COALESCE(CAST(coalesce(slav.sla_vnp_ex,'0') AS double), 0) + COALESCE(CAST(coalesce(sl.extend_sla___addon,'0') AS double), 0)+ 0.5) as double)
...
select
concat(cast(month(expected_delivery)as varchar ),'/',cast(year(expected_delivery)as varchar )) month_    --,t1.lm_tracking_no
...
from data1 
where date(expected_delivery) between date '2022-11-01' and date '2023-01-31' 
and buyer_shipping_address_state in ('Hà Nội')
group by 1,2,3
