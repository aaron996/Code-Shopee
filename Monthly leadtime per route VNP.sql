...
,data1 as (
    ...
    from data
    where date(from_unixtime(to_unixtime(pku_done_time) + cast((sla + d_sundays + d_holidays) as bigint)*3600*24)) between date '2023-02-01' and date '2023-02-28'
...
select 
        ...
        --,buyer_shipping_address_state
                ,round(avg(d1.deli_1st_leadtime),1) as "current avg_time"
                --,round(avg(d2.deli_1st_leadtime),1) as "2022 avg_time"
        ,round(approx_percentile(d1.deli_1st_leadtime,0.8),1) as "percentile_80 current timeframe"
        --,round(approx_percentile(d2.deli_1st_leadtime,0.8),1) as "percentile_80 2022 timeframe"
        ,round(approx_percentile(d1.deli_1st_leadtime,0.95),1) as "percentile_95 current timeframe"
        --,round(approx_percentile(d1.deli_1st_leadtime,0.95),1) as "percentile_95 2022 timeframe"
